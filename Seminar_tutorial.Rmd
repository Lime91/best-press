...
The rationale behind these two measures is to reward a good model fit and punish for model complexity. The **PRESS-statistic** assesses model quality in a different way. Instead of maximum-likelihood-estimation, the prediction quality for every point in the data set is estimated as follows:
$$\begin{aligned}PRESS = \sum_{i=1}^{n}(y_i-\hat{y}_{-i})^2\end{aligned}$$ where $\hat{y}_{-i}$ denotes the estimated response value for the $i$-th observation based on a model computed with this observation omitted. In other words, for every point in the data set, the prediction quality of a model which is based on the remaining $n-1$ data points is evaluated (as squared difference between the actual and predicted value). The PRESS-statistic is defined as the sum of these errors. Its name stands for **PRE**diction **S**um of **S**quares [2].
...
2. Start the algorithm with the function `bestPressDf` and pass the selected variables and the data set to the function.
